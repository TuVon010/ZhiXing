...
- **主动感知 Agent**：每封新邮件进来后自动触发一次 AI 分析，同时输出垃圾邮件评分、分类、一句话摘要、可解释理由、待办事项提取、日程事件识别、是否需要回复、优先级。用户纠偏写入记忆，下次感知时参考偏好。高置信度垃圾邮件自动标记（不删除），待办和日程以候选形式等待用户确认，日程创建前自动检测时间冲突。
- **每日收件箱摘要**：每天自动生成前一天的邮件汇总，包含邮件总数、分类统计、需回复邮件、高优先级邮件、有截止日期的待办、最紧急事项。基于感知结果，不重复调用模型。
...
在“邮箱账号”添加授权码、测试连接，再明确启用收取。新账号默认关闭自动分析；历史导入和自动分析分别控制。详细说明见 [多邮箱配置](docs/MAIL_SETUP.md)。
