...
- **待办、日程与摘要**：证据和时间可核对的本地待办、日程自动保存；含糊或冲突的事项进入“待核实建议”。日程建立提前 30 分钟的本地提醒。真实模式每日汇总前一天邮件；演示模式可手动生成。摘要复用已有感知结果，不重复调用模型。
...
- **本地记录管理**：邮件、未提交草稿、通知和其他可恢复资料可移入回收站；待办、日程和提醒的删除仍走审批，运行记录只能隐藏，Trace 与审计保留。高级管理页可盘点旧项目数据并先生成独立备份；旧 `email` 来源不能凭字段认定为测试邮件，不会自动清理。
...
**下一步工作**把本地待办、邮件跟进、待核实建议和提醒分开展示。证据明确的低风险本地事项自动保存；对外发送仍由用户最终审批。在“邮箱账号”可创建含 14 封合成邮件的测试账号，完整规则与练习路径见 [邮件工作项与审批边界](docs/MAIL_WORK_AUTOMATION.md)。
