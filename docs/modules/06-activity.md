...
页面使用 [`MailActivity.tsx`](../../frontend/src/MailActivity.tsx)；`GET /mail/activity` 由 [`mail_observability.py`](../../backend/mail_observability.py) 聚合 `assistant_turn` 和 `run`，按账号范围分页并排除 `ui_hidden`。`GET /mail/traces/{id}` 可从 Agent 轮次追到关联运行、模型调用、工具审计和费用摘要；[`MailTrace.tsx`](../../frontend/src/MailTrace.tsx) 展示时间线和 JSON 导出。隐藏接口为 `POST /mail/records/{id}/hide`，恢复显示为 `unhide`。
