...
“移入回收站”若返回 `run_id`，表示删除申请正等待审批，当前对象未立即消失；应到审批中心和 Trace 继续核对。区分 `trashed`（可恢复本地记录）、`deleted`（经动作删除）与 `ui_hidden`（仅不在运行列表展示）。测试归档 `artifacts/test-runs/` 不属于主库记录，不在此页批量清理。
