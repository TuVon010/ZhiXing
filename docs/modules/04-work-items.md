...
本地完成/重新打开走 `POST /mail/todos/{id}/{operation}`；邮件跟进处理走 `POST /mail/followups/{id}/{operation}`；候选确认与忽略走 `/mail/perception/todos/...` 或 `/mail/perception/calendars/...`。这些本地可逆操作无需每次审批。真正发送回复仍从草稿进入审批中心。每日摘要见 [`mail_digest.py`](../../backend/mail_digest.py)，生成时复用已感知结果。
