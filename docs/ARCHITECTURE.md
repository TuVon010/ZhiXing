...
当前使用流程、代码地图与调试实验见 [工作台指南](WORKSPACE_GUIDE.md)。
...
Worker 每轮尝试一次动作队列和一次邮件队列，防止邮件持续积压使动作饥饿。导入按扫描额度分批续接，累计额度满足后完成；用户暂停/取消在批间保持。IMAP 先按扩展日期区间筛 UID，再按 INTERNALDATE 精确筛选，不再对所有历史邮件逐封读取头部。

Agent 可通过 `action_status` 查询选定账号内的动作结果；等待审批时结束当前对话并引导用户确认，执行结果由通知和 Trace 持续呈现。旧 `.env` 直发路径已经移除，真实发送统一要求账号草稿及审批快照。
