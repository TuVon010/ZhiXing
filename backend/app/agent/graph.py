"""Durable LangGraph action runtime.

The graph advances a validated action plan through risk checks and controlled
tools.  Checkpoints may replay nodes, so every external side effect is also
protected by the execution ledger in ``tools.py``.
"""
...
from backend.app.agent.tools import execute, validate
...
def ingest(message, db=store, explicit_plan=None, replay=False, frozen_versions=None, prepare=None):
    """Persist one normalized request and enqueue its stable workflow id."""
