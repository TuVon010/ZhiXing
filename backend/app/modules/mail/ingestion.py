...
from backend.app.modules.mail.repository import require,secret,enqueue,initialize
from backend.app.modules.mail.schemas import ImportRequest
from backend.app.integrations.imap import open_imap
...
def connection(db,account):
    ...
    with open_imap(account, secret(db, account)) as imap:
        yield imap
...
def store_message(db,account_id,validity,mail_uid,raw,received_at,folder='INBOX',return_details=False):
    ...
    from backend.app.modules.mail.filtering import should_filter,DEFAULT_RULES
    ...
    if status in ('active','review'):
        ...
        from backend.app.modules.mail.work_items import resolve_thread_followups
