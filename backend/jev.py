...
def review_plan(db, run_id, message, plan):
    ...
    try:
        ...
        usage=result['usage']
        cost=usage['input_tokens']*settings.jev_input_price/1_000_000 if usage['input_tokens'] is not None and settings.jev_input_price is not None else None
        ...
        db.update(ident,{**body,'response':result,'flags':flags,'cost':cost,
        ...
        db.update(ident,{**body,'error':type(exc).__name__,'http_status':code,
