...
from .mail_store import require,secret
...
def smtp_connection(db,account):
    ...
    smtp=smtplib.SMTP_SSL(account['smtp_host'],account['smtp_port'],context=ctx,timeout=30) if account['smtp_tls']=='ssl' else smtplib.SMTP(account['smtp_host'],account['smtp_port'],timeout=30)
    try:
        if account['smtp_tls']=='starttls':smtp.ehlo();smtp.starttls(context=ctx);smtp.ehlo()
        smtp.login(account['username'],secret(db,account));yield smtp
    finally:
        try:smtp.quit()
