...
export function TraceView({detail}:{detail:TraceDetail}) {
 ...
 return <section className="trace-view">
  ...
  <div className="trace-metrics">{[['运行耗时（含等待）',formatMs(t.duration_ms)],['模型耗时',formatMs(t.model_latency_ms)],['事件 / 模型调用',`${t.event_count} / ${t.model_call_count}`],['输入 / 输出 Token',`${t.input_tokens??'未知'} / ${t.output_tokens??'未知'}`],['总 Token',t.total_tokens??'未知'],['费用（配置币种）',t.cost==null?'未知':t.cost.toFixed(6)]].map(([label,value])=><div key={label}><small>{label}</small><strong>{value}</strong></div>)}</div>
